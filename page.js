(function(window, document) {
    var Pagination = function(options, fn1, fn2) {
        ...
        if (typeof(fn1) !== 'function' || typeof(fn2) !== 'function') {
            console.log('fn1 和 fn2 必须是正确的回调函数')
            return
        }
        ...
        Object.defineProperty(this.output_el, "content", {
            get: function() {
                return document.getElementById(options.content).innerHTML;
            },
            set: function(el) {
                document.getElementById(options.content).innerHTML = el;
            }
        })

        Object.defineProperty(this.output_pager, 'content', {
            get: function() {
                return document.getElementById(options.pager).innerHTML;
            },
            set: function(el) {
                document.getElementById(options.pager).innerHTML = el;
            }
        })
        ...
        if (typeof(options.dataSource) == 'string') {
            this.data = JSON.parse(options.dataSource);
        } else {
    ...
    Pagination.prototype.init = function() {
        this.output_el_array = this.el_array();
        this.max = this.output_el_array.length; // 页码数
        this.set_el();
        this.set_pager();
    }

    Pagination.prototype.set_page = function(index) {
        this.num = index - 1;
        this.set_el();
    ...
    Pagination.prototype.set_el = function() {
        ...
        try {
            if (this.index < 1 || this.index > this.max) {
                throw ('默认页码 index 不能小于1，或者不能大于最大页码数，最大页码数可在 Console 查看,请修改默认页码！')
            }
        } catch (err) {
            alert(err);
            location.reload();
        ...
        var el_array_dom = this.fn1.call(this, this.output_el_array, this.num); // 回调根据数据源生成的数组
    ...
    Pagination.prototype.el_array = function() {
        ...
        for (var i = 0; i < data.length; i++) {
            limit.push(i);
            if (limit.length == perpage) {
                output.push(data.slice(m, i + 1));
                limit.splice(0, limit.length); // 清空 limit数组
                m = i + 1;
            }
        }

        if (data.length % perpage > 0) {
            var end = -(data.length % perpage); // 余下子元素
